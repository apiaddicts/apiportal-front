...
function Accordion({ title, body, active, setActive }) {
  ...
  return (
    <div className={classes.accordion}>
      <div className={active.item === title || flag ? `${classes.accordion__head} ${classes.active}` : `${classes.accordion__head}`} onClick={() => { setFlag(!flag); }} role='button' tabIndex='0'>
        <div className={classes.accordion__head__title}>
          <p>{title}</p>
        </div>
        <div className={classes.accordion__head__title__actionbutton}>
          {active.item === title || flag ? <Icon id='MdExpandLess' /> : <Icon id='MdExpandMore' />}
        </div>
      </div>
      {
        active.item === title && (
